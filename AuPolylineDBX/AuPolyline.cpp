...
Acad::ErrorStatus AuPolyline::subGetOsnapPoints(AcDb::OsnapMode osnapMode, Adesk::GsMarker gsSelectionMark,
...
{
    assertReadEnabled();
    return (AcDbPolyline::subGetOsnapPoints(
        osnapMode, gsSelectionMark, pickPoint, lastPoint, viewXform, snapPoints, geomIds));
}

Acad::ErrorStatus AuPolyline::subGetOsnapPoints(AcDb::OsnapMode osnapMode, Adesk::GsMarker gsSelectionMark,
    const AcGePoint3d &pickPoint, const AcGePoint3d &lastPoint, const AcGeMatrix3d &viewXform,
    AcGePoint3dArray &snapPoints, AcDbIntArray &geomIds, const AcGeMatrix3d &insertionMat) const
{
    assertReadEnabled();
    return (AcDbPolyline::subGetOsnapPoints(
...
Acad::ErrorStatus AuPolyline::subGetGripPoints(AcDbGripDataPtrArray &grips, const double curViewUnitSize,
...
Acad::ErrorStatus AuPolyline::getClosestPointTo(
    const AcGePoint3d &givenPnt, AcGePoint3d &pointOnCurve, Adesk::Boolean extend /*=Adesk::kFalse*/) const
{
    assertReadEnabled();
    return (AcDbPolyline::getClosestPointTo(givenPnt, pointOnCurve, extend));
}

Acad::ErrorStatus AuPolyline::getClosestPointTo(const AcGePoint3d &givenPnt, const AcGeVector3d &direction,
...
Acad::ErrorStatus AuPolyline::getProjectedCurve(
    const AcGePlane &plane, const AcGeVector3d &projDir, AcDbCurve *&projCrv) const
{
    assertReadEnabled();
...
Acad::ErrorStatus AuPolyline::getOffsetCurvesGivenPlaneNormal(
    const AcGeVector3d &normal, double offsetDist, AcDbVoidPtrArray &offsetCurves) const
{
    assertReadEnabled();
